...
import handler from '@/pages/api/account/settings/notifications';
...
describe('Account Settings API - Notifications', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('should validate notification settings object', async () => {
    const { req, res } = createMocks({
      method: 'POST',
    ...
    expect(res._getJSONData()).toEqual({ message: 'Invalid notification settings' });
  });
