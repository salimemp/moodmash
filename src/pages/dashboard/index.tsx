...
interface StreamingNotification {
  ...
  message: string;
  timestamp: string;
  [key: string]: unknown; // Allow other properties with unknown type instead of any
}

// Define type for activity data
...
interface DashboardData {
  ...
  recentActivity?: MoodActivityData[];
}
...
export default function Dashboard() {
  ...
  // Notifications state
  const [notifications, setNotifications] = useState<StreamingNotification[]>([]);
  ...
  } = useFetch('/api/dashboard/stats', { revalidateOnFocus: true });
  ...
  const refreshAllData = useCallback(async () => {
    try {
      await Promise.all([refetchPreferences(), refreshDashboardData()]);
      toast.success('Data refreshed successfully');
    } catch {
      // Catch error without defining a variable
            ...
            <Card>
              ...
              <CardContent>
                {dashboardError ? (
                  ...
                  <div className="space-y-2">
                    <p>Moods shared: {(dashboardData as DashboardData).moodCount || 0}</p>
                    <p>Likes received: {(dashboardData as DashboardData).likesReceived || 0}</p>
                    <p>Comments received: {(dashboardData as DashboardData).commentsReceived || 0}</p>
                  </div>
                ) : (
                ...
                <div className="space-y-4">
                  <div className="flex justify-between items-center">
                    <span className="text-sm">
                      {streamConnected ? (
                        <span className="text-green-500">Connected to real-time updates</span>
                      ) : (
                        <span className="text-red-500">Not connected</span>
                      )}
                    </span>

                    <Button size="sm" onClick={triggerNotification} disabled={!streamConnected}>
                      Test
                    </Button>
                  </div>
